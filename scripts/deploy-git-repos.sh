...
get_ssh_key() {
    ...
    SSH_KEY_FILE="/tmp/vm_ssh_key"
    
    if az keyvault secret show --name "vm-ssh-private-key" --vault-name "$KEY_VAULT_NAME" --query value -o tsv > "$SSH_KEY_FILE" 2>/dev/null; then
...
test_ssh_connectivity() {
    ...
    if ssh -i "$SSH_KEY_FILE" -o StrictHostKeyChecking=no -o ConnectTimeout=10 vmuser@$vm_ip "echo 'SSH connection successful'" &>/dev/null; then
...
install_nodejs() {
    ...
    ssh -i "$SSH_KEY_FILE" -o StrictHostKeyChecking=no vmuser@$vm_ip << 'EOF'
        # Update package list
        sudo apt-get update
...
deploy_frontend() {
    ...
    ssh -i "$SSH_KEY_FILE" -o StrictHostKeyChecking=no vmuser@$WEB_VM_PUBLIC_IP << EOF
...
deploy_backend() {
    ...
    ssh -i "$SSH_KEY_FILE" -o StrictHostKeyChecking=no vmuser@$WEB_VM_PUBLIC_IP << EOF
        ...
        ssh -i ~/.ssh/vm_key -o StrictHostKeyChecking=no vmuser@$BACKEND_VM_PRIVATE_IP << 'BACKEND_EOF'
            # Remove existing directory if it exists
            sudo rm -rf $BACKEND_DIR
            
            # Create application directory
            sudo mkdir -p $BACKEND_DIR
            sudo chown vmuser:vmuser $BACKEND_DIR
...
deploy_database() {
    ...
    ssh -i "$SSH_KEY_FILE" -o StrictHostKeyChecking=no vmuser@$WEB_VM_PUBLIC_IP << EOF
        ...
        ssh -i ~/.ssh/vm_key -o StrictHostKeyChecking=no vmuser@$DB_VM_PRIVATE_IP << 'DB_EOF'
            # Update package list
            sudo apt-get update
...
setup_ssh_bastion() {
    ...
    scp -i "$SSH_KEY_FILE" -o StrictHostKeyChecking=no "$SSH_KEY_FILE" vmuser@$WEB_VM_PUBLIC_IP:~/.ssh/vm_key
    
    ssh -i "$SSH_KEY_FILE" -o StrictHostKeyChecking=no vmuser@$WEB_VM_PUBLIC_IP "chmod 600 ~/.ssh/vm_key"
...
show_deployment_summary() {
    ...
    echo ""
    echo "SSH Access:"
    echo "  🔑 Web VM: ssh -i $SSH_KEY_FILE vmuser@$WEB_VM_PUBLIC_IP"
    echo ""
    echo "To monitor backend processes:"
    echo "  ssh -i $SSH_KEY_FILE vmuser@$WEB_VM_PUBLIC_IP"
    echo "  ssh vmuser@$BACKEND_VM_PRIVATE_IP"
    echo "  pm2 status"
    echo "  pm2 logs"
    echo ""
...
main() {
    ...
    ssh -i "$SSH_KEY_FILE" -o StrictHostKeyChecking=no vmuser@$WEB_VM_PUBLIC_IP \
        "ssh -i ~/.ssh/vm_key -o StrictHostKeyChecking=no vmuser@$BACKEND_VM_PRIVATE_IP 'curl -fsSL https://deb.nodesource.com/setup_18.x | sudo -E bash - && sudo apt-get install -y nodejs build-essential git && sudo npm install -g pm2'"
